...
from robot_sandbox.storage.case_store import append_case, find_case, load_cases, write_audio_bytes
...
class OfflineSimulationApiTest(unittest.TestCase):
    def test_simulate_text_does_not_persist_synthetic_case(self) -> None:
        with tempfile.TemporaryDirectory() as tmp, patch("robot_sandbox.web.server.DATA_DIR", Path(tmp)), patch(
...
class ModelStudioSelectionUsageTest(unittest.TestCase):
    def test_router_and_cloud_config_follow_model_studio_lv_selection(self) -> None:
        ...
        with patch("robot_sandbox.web.server.fetch_model_studio_selection", return_value=selection):
