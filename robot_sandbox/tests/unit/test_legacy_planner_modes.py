...
from robot_sandbox.legacy.planner import LlmTaskPlanner, PlannerError, build_task_planner
...
class PlannerModesTest(unittest.TestCase):
    def test_llm_planner_parses_openai_style_json_content(self) -> None:
        ...
        with patch("robot_sandbox.legacy.planner.requests.post", return_value=response):
    ...
    def test_hybrid_planner_falls_back_to_rule(self) -> None:
        ...
        with patch("robot_sandbox.legacy.planner.requests.post", side_effect=RuntimeError("planner offline")):
            plan = planner.plan("前进")
        self.assertIsNotNone(plan)
        assert plan is not None
        self.assertEqual(plan.skill_id, "move_forward")
