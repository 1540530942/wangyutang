...
from robot_sandbox.web import server
from robot_sandbox.tools.executors import execute_planned_task
from robot_sandbox.harness.react_loop import route_transcript, transcribe_audio_path
from robot_sandbox.legacy.planner import build_task_planner
...
class AudioRegressionSuite(unittest.TestCase):
    ...
    def test_execute_planned_task_routes_action(self) -> None:
        ...
        with patch("robot_sandbox.tools.executors.create_action_task", return_value={"task": {"id": "t1"}}) as create_action_task:
    ...
    def test_execute_planned_task_routes_face(self) -> None:
        ...
        with patch("robot_sandbox.tools.executors.create_face_task", return_value={"state": {"emotion": "happy"}}) as create_face_task:
            result = execute_planned_task(
                plan,
                "笑一笑",
    ...
    def test_api_recognize_text_exposes_plan_and_skill(self) -> None:
        with TestClient(server.app) as client, patch("robot_sandbox.web.server.ACTION_SERVER", "http://action.local"), patch(
            "robot_sandbox.web.server.route_transcript",
            return_value={
