...
- For 直到X / 一直走到X / keep moving until X instructions: one move_forward = 10cm only — far from "碰到". Use this exact sequence to stay within 8 turns: (1) front_distance → if failed: inspect_scene → move_forward. (2) front_distance → move_forward (skip inspect_scene; the safety guard reuses the cached inspect_scene from step 1 within 30s). (3) front_distance → move_forward. (4) finish. After each front_distance: if distance < 20cm, call finish immediately. If safety guard rejects move_forward, call finish. NEVER call inspect_scene more than once for 直到 loops — re-calling wastes turns and the cache is still valid.
