...
- For 直到X / 一直走到X / keep moving until X instructions: one move_forward = 10cm only — far from "碰到". Required sequence: front_distance → move_forward → front_distance → [if ≥ 20cm or failed: move_forward again] → front_distance → [if ≥ 20cm or failed: move_forward again] → finish. After every move_forward you MUST call front_distance immediately. NEVER call finish right after move_forward without calling front_distance first. Stop and call finish only when: (a) front_distance returns available and distance < 20cm (reached), OR (b) safety guard rejects move_forward (too close), OR (c) 3 move_forwards have been executed.
