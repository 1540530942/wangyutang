...
class SparkQwenVisionClient:
    ...
    def health(self) -> dict[str, object]:
        try:
            ...
            raise HTTPException(status_code=502, detail=f"Spark Qwen vision health check failed: {exc}") from exc
        return {
    ...
    def models(self) -> dict[str, object]:
        return {
            "object": "list",
            "provider": "spark_qwen_vision",
            "data": [{"id": settings.spark_qwen_model, "object": "model", "capability": "vision_understanding"}],
        }
    ...
    def analyze_image_bytes(self, data: bytes, question: str, model: str | None = None, filename: str = "") -> dict[str, Any]:
        ...
        selected_model = model or settings.spark_qwen_model
        resized_data, image_info = _resize_image(data)
        mime = _image_mime(resized_data, "image.jpg")
        ...
        raise HTTPException(status_code=502, detail=f"Spark Qwen vision request failed: {last_exc}")


vision_with_spark_qwen = SparkQwenVisionClient()
